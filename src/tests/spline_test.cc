...
TEST_F(CubicSplineTest, Constructor_MinimumPoints) {
    ...
    EXPECT_NO_THROW({
        CubicSplineInterpolator spline(two_points);
    });
}
...
TEST_F(CubicSplineTest, Constructor_InsufficientPoints) {
    ...
    EXPECT_THROW({
        CubicSplineInterpolator spline(one_point);
    }, std::runtime_error);
}
...
TEST_F(CubicSplineTest, Constructor_NonIncreasingX) {
    ...
    EXPECT_THROW({
        CubicSplineInterpolator spline(bad_x);
    }, std::runtime_error);

    std::vector<s21::SplinePoint> decreasing = {{2.0, 0.0}, {1.0, 1.0}};
    EXPECT_THROW({
        CubicSplineInterpolator spline(decreasing);
    }, std::runtime_error);
}
...
TEST_F(CubicSplineTest, Interpolate_QuadraticFunction) {
    ...
    EXPECT_TRUE(almost_equal(spline.interpolate(0.5), 0.25, 1e-2));  // может быть небольшая погрешность
    EXPECT_TRUE(almost_equal(spline.interpolate(1.5), 2.25, 1e-2));
    EXPECT_TRUE(almost_equal(spline.interpolate(2.5), 6.25, 1e-2));
}
...
TEST_F(CubicSplineTest, Interpolate_CubicFunction) {
    ...
    // Между узлами
    EXPECT_TRUE(almost_equal(spline.interpolate(0.5), 0.125, 5e-2));
    EXPECT_TRUE(almost_equal(spline.interpolate(1.5), 3.375, 5e-2));
    EXPECT_TRUE(almost_equal(spline.interpolate(2.5), 15.625, 5e-2));
}
