...
"Yes, the implementation of the simplest interpolation and approximation methods. But I still don't understand what it's all for. He mentioned something about investments and customers, but he seemed more interested in what was happening on his screen," Eve replied to Dave, the lead developer in the finance department she had been assigned to help. Bob mentioned in his conversation with Eve that they were currently understaffed and had a lot of pressing work to do.

"That's true. But it's not really something we need right now — it's more of an order from the AID department."

"Wait, AID? What kind of department is that? I mean, we have a big company on every continent, but I have never heard of that department."

"It's funny you don't know. It's an Assistive Intelligence Development Department. AI assistants for short.  Alice runs it, by the way, with Charlie's help."

"Alice? But she was just doing simple object recognition on video for surveillance systems."

"Not for a year. Not many people know about the AID, of course: NDA, corporate secrets, billions of dollars at stake, and all that. But you seemed to talk to her quite a lot, so... I don't know," Dave stopped his apparently unsuccessful attempts to comfort an upset and confused Eve.  She struggled to figure out why her friend hadn't said anything.

"So, yes, about the assignment," Dave continued as if nothing had happened. "We've been asked to write a module that can assist and guide clients in their bidding..." 

Eve was no longer really paying attention to what Dave was saying. All she cared about was the unexpected news.  Although it explained Alice's recent behavior, some things were still unclear and needed to be clarified.

## Introduction

In this project you will learn about interpolation and approximation algorithms. Using these algorithms, you will be able to predict the probabilistic behavior of stock prices and their forecasts for the foreseeable future based on historical data without regard to external factors __*__.
...
Function interpolation is used when you want to find the value of the function $`y(х)`$ at the value of the argument $`x`$ that belongs to the interval $`[x_0, ..., x_n]`$ ($`x_0, ..., x_n`$ are values given in the table), but does not coincide with any of the known values of that interval.

In many cases, the analytic expression of the function $`y(x)`$ is not known and it is not possible to find the value of this function from the table of its values.
Instead, another function $`f(x)`$ is plotted, which is easy to calculate and has the same value table (coinciding with it at the points $`x_0, x_1, x_2,..., x_n`$) as $`y(x)`$.

Finding an approximate function is called interpolation and the points $`x_0, x_1, x_2, ..., x_n`$ are interpolation nodes. Usually the interpolating function is found as a polynomial of $`n`$ degree.
...
*Approximation is the replacement of some mathematical objects with others that are, in one sense or another, close to the original ones.

When interpolating, the interpolating function passes strictly through the table nodes because the number of coefficients in the interpolating function is equal to the number of table values.
Approximation is a method in which the approximated function does not pass through the interpolation nodes, but instead passes between them to find additional values that are different from the table data.

If the analytical expression of the function is unknown or very complex, then we must find an empirical formula whose values would not differ much from the experimental data.
...
- There can be up to 5 graphs with the same value of the number of days `M` displayed in the field at the same time (all graphs should have different color).
